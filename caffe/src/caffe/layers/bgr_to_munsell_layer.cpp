...
void BgrToMunsellLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  top[0]->ReshapeLike(*bottom[0]);
}

template <typename Dtype>
void BgrToMunsellLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void BgrToMunsellLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(BgrToMunsellLayer);
#endif

INSTANTIATE_CLASS(BgrToMunsellLayer);
REGISTER_LAYER_CLASS(BgrToMunsell);
